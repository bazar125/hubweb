<template>
  <ul class="pagination">
    ...
    <template v-if="notEnoughPages">
      <li class="page-item" v-for="n in totalPage" :key="n" :class="{'active': isCurrentPage(n)}">
        <a class="page-link" @click.prevent="loadPage(n)" v-html="n"></a>
      </li>
    </template>
    <template v-else>
      <li class="page-item" v-for="n in windowSize" :key="n" :class="{'active': isCurrentPage(windowStart+n-1)}">
